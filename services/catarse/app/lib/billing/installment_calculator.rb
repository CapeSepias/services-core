...
module Billing
  class InstallmentCalculator
    attr_reader :interest_rate, :minimum_amount_for_installment
    ...
    def initialize
      ...
      @minimum_amount_for_installment =
        CatarseSettings.get_without_cache(:pagarme_minimum_amount_for_installment).to_i * 100
    end

    def calculate(amount:, installments_count:)
      return if amount < minimum_amount_for_installment || installments_count > @max_installments

      simulation_for_installment(amount: amount, installments_count: installments_count)
    end

    def simulations(amount:)
      max_installments = amount < minimum_amount_for_installment ? 1 : @max_installments
