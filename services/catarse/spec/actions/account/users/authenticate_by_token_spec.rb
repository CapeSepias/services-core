...
RSpec.describe Account::Users::AuthenticateByToken, type: :actor do
  describe 'Inputs' do
  ...
  describe '#call' do
    ...
    let(:auth_token) { Account::AuthToken.encode(payload: payload, expires_at: expires_at) }
    ...
    context 'when auth token is invalid' do
      before { allow(Account::AuthToken).to receive(:decode).and_raise(JWT::VerificationError) }
